...
TEST(Tuple, PointConstruct) {
    ...
    EXPECT_FLOAT_EQ(t[1], 2.1);
    EXPECT_FLOAT_EQ(t[2], 3.4);
    EXPECT_FLOAT_EQ(t[3], 1);
}

TEST(Tuple, VectorConstruct) {
    tuple t = {2.3, 3.4, 2.6, 0};
    EXPECT_FLOAT_EQ(t[0], 2.3);
    EXPECT_FLOAT_EQ(t[1], 3.4);
    EXPECT_FLOAT_EQ(t[2], 2.6);
    EXPECT_FLOAT_EQ(t[3], 0);
}

TEST(Tuple, Factory) {
    auto point = pointFactory(2.3, 3.4, 4.5);
    EXPECT_FLOAT_EQ(point[0], 2.3);
    EXPECT_FLOAT_EQ(point[1], 3.4);
    EXPECT_FLOAT_EQ(point[2], 4.5);
    EXPECT_FLOAT_EQ(point[3], 1);
    auto vector = vectorFactory(3.5, 1.2, 5.6);
    EXPECT_FLOAT_EQ(vector[0], 3.5);
    EXPECT_FLOAT_EQ(vector[1], 1.2);
    EXPECT_FLOAT_EQ(vector[2], 5.6);
    EXPECT_FLOAT_EQ(vector[3], 0);
}
...
TEST(Tuple, AddOp) {
    ...
    auto er = point + point;
    EXPECT_FLOAT_EQ(p[0], 3.2);
    EXPECT_FLOAT_EQ(p[1], 3.4);
    EXPECT_FLOAT_EQ(p[2], 3.6);
    EXPECT_TRUE(p[3] == 1);
    EXPECT_FLOAT_EQ(v[0], 4.2);
    EXPECT_FLOAT_EQ(v[1], 4.4);
    EXPECT_FLOAT_EQ(v[2], 4.6);
    EXPECT_TRUE(v[3] == 0);
    EXPECT_TRUE(er[3] == 2);
    // +=
    vector += v;
    EXPECT_FLOAT_EQ(vector[0], 6.3);
    EXPECT_FLOAT_EQ(vector[1], 6.6);
    EXPECT_FLOAT_EQ(vector[2], 6.9);
...
TEST(Tuple, SubOp) {
    ...
    // -=
    point -= vector;
    EXPECT_FLOAT_EQ(point[0], -1.0);
    EXPECT_FLOAT_EQ(point[1], -1.0);
    EXPECT_FLOAT_EQ(point[2], -1.0);
...
TEST(Tuple, MulOp) {
    ...
    float factor = 0.5;
    auto p = point * factor;
    EXPECT_FLOAT_EQ(p[0], 0.55);
    EXPECT_FLOAT_EQ(p[1], 0.6);
    EXPECT_FLOAT_EQ(p[2], 0.65);
    EXPECT_FLOAT_EQ(p[3], 0.5);
    // *=
    vector *= factor;
    EXPECT_FLOAT_EQ(vector[0], 1.05);
    EXPECT_FLOAT_EQ(vector[1], 1.1);
    EXPECT_FLOAT_EQ(vector[2], 1.15);
    EXPECT_TRUE(vector[3] == 0);
}
...
TEST(Tuple, Magnitude) {
    ...
    EXPECT_FLOAT_EQ(glm::length(vector), std::sqrt(14));
